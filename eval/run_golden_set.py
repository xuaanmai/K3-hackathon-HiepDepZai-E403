...
def run_case(case: dict[str, Any], pages_by_lesson: dict[str, list[dict]]) -> dict:
    ...
    error = None
    try:
        if is_blocked_out_of_scope(case["question"]):
            response = TutorResponse(
                decision="not_found",
                answer=None,
                citations=[],
            ...
            while True:
                ...
                try:
                    response = answer_question(
                        case["question"], sources, history=history
                    )
                    break
                except TutorError as exc:
                    message = str(exc)
                    transient = (
                        "429 RESOURCE_EXHAUSTED" in message
                        or "503 UNAVAILABLE" in message
                        or "10054" in message
                    )
                    if not transient or api_attempts >= 4:
                        raise
                    retry_match = re.search(
                        r"retry in ([0-9.]+)s", message, flags=re.IGNORECASE
                    )
                    if retry_match:
                        wait_seconds = float(retry_match.group(1)) + 3
                    elif "503 UNAVAILABLE" in message:
                        wait_seconds = 15 * api_attempts
                    else:
                        wait_seconds = 10 * api_attempts
                    print(
                        f"{case['id']}: transient API error; retry "
                        f"{api_attempts + 1} in {wait_seconds:.0f}s",
                        flush=True,
                    )
